...
`esy-gdbm` is package, [`gdbm`](https://www.gnu.org/software/gdbm/), packaged for [`esy`](https://esy.sh/).
...
`esy-gdbm` can be used from both NPM and directly from Github.

### From NPM 

`esy-gdbm` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-gdbm` to install it, or specify it in
...
{
  "dependencies": {
    "esy-gdbm": "esy-packages/esy-gdbm"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-gdbm": "esy-packages/esy-gdbm#<commit hash>"
  }
```
